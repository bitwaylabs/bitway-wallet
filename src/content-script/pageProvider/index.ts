...
const channelName = script?.getAttribute('channel') || 'bitway';
...
export class BitwayProvider extends EventEmitter {
...
declare global {
  interface Window {
    bitway: BitwayProvider;
  }
}

const provider = new BitwayProvider();

if (!window.bitway) {
  window.bitway = new Proxy(provider, {
    deleteProperty: () => true
  });
}

Object.defineProperty(window, 'bitway', {
...
window.dispatchEvent(new Event('bitway#initialized'));
