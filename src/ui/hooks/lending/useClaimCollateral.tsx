...
import { bitwayLendingMessageComposer } from '@/codegen/src';
...
export function useClaimCollateral(loan?: Loan) {
  ...
  const claim = async ({ feeRate }: { feeRate: number }) => {
    try {
      ...
      const msg = bitwayLendingMessageComposer.withTypeUrl.redeem({
      ...
      if (hashResponse.tx_response.code === 0) {
        ...
        queryClient.invalidateQueries({ queryKey: ['getDepositTxByCollateralAddress'] });
