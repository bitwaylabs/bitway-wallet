...
function PoolItemFC({ item }: { item: PoolDataItem }) {
  ...
  const data = [
    ...
    {
      label: 'Borrow APR',
      value: `${new BigNumber(item.baseData.config.tranches[0].borrow_apr)
        .multipliedBy(100)
        .toFixed(2)}% - ${new BigNumber(
    ...
    {
      label: 'Max LTV',
      value: item?.baseData?.config?.max_ltv ? `${item.baseData.config.max_ltv * 100}%` : '-',
