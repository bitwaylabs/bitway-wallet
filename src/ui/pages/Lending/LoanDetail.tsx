...
export default function LoanDetailScreen() {
  ...
  const { dlcEvent } = useGetDlcEventById(loan?.dlc_event_id);
  const { attestation } = useGetAttestationByEventId({ eventId: loan?.dlc_event_id });
  ...
  const dataLoan = [
    ...
    },
    {
      ...
      tip: 'The transaction (on Bitway) that delivered the loan amount to your wallet'
    },
    {
      label: 'Repay Tx',
      value: (
      ...
      tip: 'The transaction (on Bitway) you used to repay the loan'
    }
  ];
      ...
      value: (
        <Text
          ...
          {lendingPool?.pool?.config?.liquidation_threshold
            ? `${lendingPool?.pool?.config?.liquidation_threshold * 100}%`
            : '-'}
        </Text>
      ),
    ...
    },
    {
      ...
      value: (
        <Text
          ...
          {`${(liquidationParams?.params.liquidation_bonus_factor || 0) * 100}%`}
        </Text>
      ),
  ...
  const dataOracle = [
    {
      ...
      value: (
        <Text
          ...
          {formatAddress(dlcEvent?.event.pubkey || '', 4)}
        </Text>
      )
    },
    {
      ...
      value: (
        <>
          ...
          <Text
            ...
            {dlcEvent?.event.outcomes[1] ? formatTimeWithUTC(+dlcEvent?.event.outcomes[1] * 1000) : '-'}
          </Text>
          <Box
            sx={{
              width: '2px',
    ...
    },
    {
      ...
      value: (
        <Text
          ...
          {!dlcEvent || dlcEvent?.event.outcome_index < 0 ? '-' : dlcEvent?.event.outcome_index}
        </Text>
      )
    },
    {
      ...
      value: (
        <Text
          ...
          {!dlcEvent || dlcEvent?.event.outcome_index < 0 ? '-' : formatTimeWithUTC(dlcEvent.event.trigger_at)}
        </Text>
      )
    },
    {
      ...
      value: (
        <Text
          ...
          {!dlcEvent || dlcEvent?.event.outcome_index < 0
            ? '-'
            : formatAddress(attestation?.attestation.signature || '', 4)}
        </Text>
      )
    }
  ];
    ...
    <Layout>
      ...
      <Content
        ...
        <Column gap={'md'}>
          <Row justifyCenter>
            <Stack
              ...
              <Typography
                ...
                onClick={() => {
                  window.open(`${SIDE_STATION_URL}/lending/markets/loan/${loan.vault_address}`);
                }}>
                {formatAddress(loan.vault_address, 6)}
              </Typography>
          ...
          {dataOracle.map((item, index) => {
            return (
              <Row key={index} full justifyBetween itemsCenter>
                <LightTooltip title={''} arrow placement="top">
                  <Typography
                    sx={{
                      fontSize: '12px',
                      color: colors.grey12,
      ...
      {getDepositInfoSuccess && (
        <>
          ...
          ) : loan.status === 'Rejected' && redeemEnable && totalDepositUnitAmount !== claimedUnitAmount ? (
            <Button
              preset="primary"
          ...
          ) : loan.status === 'Cancelled' && redeemEnable && totalDepositUnitAmount !== claimedUnitAmount ? (
            <Button
              preset="primary"
              style={{ display: 'flex', position: 'fixed', bottom: 16, left: 16, right: 16 }}
              onClick={() => {
