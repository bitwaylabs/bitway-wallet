...
export class QueryClientImpl implements Query {
  ...
  params(request: QueryParamsRequest = {}): Promise<QueryParamsResponse> {
    ...
    const promise = this.rpc.request("bitway.incentive.Query", "Params", data);
    return promise.then(data => QueryParamsResponse.decode(new BinaryReader(data)));
  }
  rewards(request: QueryRewardsRequest): Promise<QueryRewardsResponse> {
    const data = QueryRewardsRequest.encode(request).finish();
    const promise = this.rpc.request("bitway.incentive.Query", "Rewards", data);
  ...
  rewardStats(request: QueryRewardStatsRequest = {}): Promise<QueryRewardStatsResponse> {
    ...
    const promise = this.rpc.request("bitway.incentive.Query", "RewardStats", data);
